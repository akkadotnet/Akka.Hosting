...
public abstract class AllTestForEventFilterBase<TLogEvent> : EventFilterTestBase where TLogEvent : LogEvent
{
    ...
    protected AllTestForEventFilterBase(LogLevel logLevel)
        : base(logLevel)
    {
    }
    ...
    public void Single_message_is_intercepted()
    {
        if (_testingEventFilter is null)
            throw new NullReferenceException(
                "_testingEventFilter should not be null, check CreateTestingEventFilter implementation.");
    ...
    public void Can_intercept_messages_when_source_is_specified()
    {
        ...
        _testingEventFilter.ForLogLevel(LogLevel, source: LogSource.FromType(GetType(), Sys))
            .ExpectOne(() => LogMessage("whatever"));
        TestSuccessful = true;
    }
    ...
    {
        this.Invoking(_ => { EventFilter.Error().Expect(0, () => { Log.Error("something"); }); }).Should()
            .Throw<Exception>("Expected 0 events");
    }
    ...
    public async Task ExpectAsync_should_await_actionAsync()
    {
        ...
        await Assert.ThrowsExceptionAsync<AssertFailedException>(async () =>
        {
    ...
    public void Should_timeout_if_too_few_messages()
    {
        ...
        Invoking(() =>
        {
            _testingEventFilter.ForLogLevel(LogLevel)
                .Expect(2, TimeSpan.FromMilliseconds(50), () => { LogMessage("whatever"); });
        }).Should().Throw<AssertFailedException>().WithMessage("*Timeout*");
    }
