...
{
    public class ReceiveTests : HostingSpec
    {
        public ReceiveTests(ITestOutputHelper output) : base(nameof(ReceiveTests), output)
        {
        }
        ...
        public async Task ReceiveNAsync_should_receive_correct_number_of_messages()
        {
            ...
            messages.Should().BeEquivalentTo(new[] { "1", "2", "3" }, opt => opt.WithStrictOrdering());
            messages = await ReceiveNAsync(1).ToListAsync();
            messages.Should().BeEquivalentTo(new[] { "4" });
        }
        ...
        public async Task FishForMessageAsync_should_return_matched_message()
        {
            ...
            TestActor.Tell(10);
            TestActor.Tell(20);
            (await FishForMessageAsync<int>(i => i >= 10)).Should().Be(10);
        }
        ...
        public async Task ReceiveWhileAsync_Filter_should_break_on_function_returning_null_and_return_correct_messages()
        {
            ...
            TestActor.Tell(4);
            var messages = await ReceiveWhileAsync(_ => _ is double ? null : _.ToString()).ToListAsync();
            messages.Should().BeEquivalentTo(new[] { "1", "2", "3" }, opt => opt.WithStrictOrdering());
        }
        ...
        public async Task ReceiveWhileAsync_Predicate_should_break_when_predicate_returns_false_and_return_correct_messages()
        {
            ...
            TestActor.Tell("-----------");
            TestActor.Tell("4");
            var messages = await ReceiveWhileAsync<string>(s => s.Length == 1).ToListAsync();
            messages.Should().BeEquivalentTo(new[] { "1", "2", "3" }, opt => opt.WithStrictOrdering());
        }
        ...
        public async Task ReceiveWhileAsync_Predicate_should_break_when_type_is_wrong_and_we_dont_ignore_those_and_return_correct_messages()
        {
            ...
            TestActor.Tell(4);
            TestActor.Tell("5");
            var messages = await ReceiveWhileAsync<string>(s => s.Length == 1, shouldIgnoreOtherMessageTypes: false).ToListAsync();
            messages.Should().BeEquivalentTo(new[] { "1", "2", "3" }, opt => opt.WithStrictOrdering());
        }
        ...
        public async Task ReceiveWhileAsync_Predicate_should_continue_when_type_is_other_but_we_ignore_other_types_and_return_correct_messages()
        {
            ...
            TestActor.Tell(4);
            TestActor.Tell("5");
            var messages = await ReceiveWhileAsync<string>(s => s.Length == 1, shouldIgnoreOtherMessageTypes: true).ToListAsync();
            messages.Should().BeEquivalentTo(new[] { "1", "2", "3", "5" }, opt => opt.WithStrictOrdering());
        }
        ...
        public async Task ReceiveWhileAsync_Predicate_should_not_consume_last_message_that_didnt_match()
        {
            ...
            var messages = await ReceiveWhileAsync<object>(_ => _ is string).ToListAsync();
            messages.Should().BeEquivalentTo(new[] { "1", "2" }, opt => opt.WithStrictOrdering());
            await ExpectMsgAsync(4711);

            messages = await ReceiveWhileAsync<object>(_ => _ is string).ToListAsync();
            messages.Should().BeEquivalentTo(new[] { "3", "4", "5" }, opt => opt.WithStrictOrdering());
