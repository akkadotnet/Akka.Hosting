...
internal class TestConsumerEntity : ReceiveActor
{
    ...
    public TestConsumerEntity(string entityId, IActorRef consumerController, IRequiredActor<AggregateActor> reqAggregator)
    {
        _entityId = entityId;
...
internal class MessageExtractor() : HashCodeMessageExtractor(10)
{
    public override string EntityId(object message) =>
        message is Job cmd ? (cmd.Payload % 10).ToString() : string.Empty;
}
...
internal class ProducerActor : ReceiveActor, IWithStash
{
    ...
    private void Active()
    {
        ...
        Receive<Job>(msg =>
        {
            _log.Info(">>>> Sending {0}", msg.Payload);
        });
